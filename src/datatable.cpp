...
// 撰写对应的removeSample()
void DataTable::removeSample(double x, double y)
{
    removeSample(DataPoint(x, y));
}

void DataTable::removeSample(std::vector<double> x, double y)
{
    removeSample(DataPoint(x, y));
}

void DataTable::removeSample(DenseVector x, double y)
{
    removeSample(DataPoint(x, y));
}
...
void DataTable::removeSample(const DataPoint &sample)
{
    // Check if the sample exists in the set
    if (samples.count(sample) > 0)
    {
        // Remove the sample from the set
        samples.erase(sample);

        // Decrease the number of duplicates if necessary
        if (numDuplicates > 0)
        {
            numDuplicates--;
        }
    }
    unrecordGridPoint(sample);
}
...
void DataTable::unrecordGridPoint(const DataPoint &sample)
{
    for (unsigned int i = 0; i < getNumVariables(); i++)
    {
        // Check if the value exists in the set
        if (grid.at(i).count(sample.getX().at(i)) > 0)
        {
            // Remove the value from the set
            grid.at(i).erase(sample.getX().at(i));
        }
    }
}
